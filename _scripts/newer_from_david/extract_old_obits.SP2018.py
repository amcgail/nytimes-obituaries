...
knownAttributes = [x[0] for x in knownAttributes] + ["ISSN","AUTHOR","HTTP"]
...
people = glob.glob( path.join( dataDir, "NYT Obituary Project, Independent Study", "NYT Obituary Project, Independent Study", "*" ) )

for personf in people:

    if "javier" in personf:
        continue

    if "michelle" not in personf:
        ...
        for datef in glob.glob(path.join(personf, "*")):
            for obitf in glob.glob(path.join(datef, "*.txt")):
