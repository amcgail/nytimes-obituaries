...
# This guy didn't really work. It seems to key on things that don't matter at all
# It's probably a function of our whatTheyWere and whatTheyDid algorithms not living up to anything :P
# I'll work on this
...
text_docs = [
    list( chain.from_iterable( simple_preprocess(x) for x in doc['whatTheyWere'] + doc['whatTheyDid']) )
    for doc in docs
]

occ.nice_occ = lambda x: x
...
for i in range(10):
    ...
    for doc_i in nearest:
        print(" - %s..." % (docs[doc_i]['fullBody'][:100]))
        print(" --- OCC: %s, similarity: %s" % ( [ occ.codeToName[x] for x in occ.nice_occ(docs[doc_i]['OCC']) ], distances[doc_i]) )
        print(" --- %s" % list(map(lambda x: dictionary.id2token[x[0]], vector_docs[doc_i])))
